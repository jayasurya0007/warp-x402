...
const Warp402Factory_1 = require("../deploy/Warp402Factory");
function parseArgs() {
    const args = {};
...
async function deploy() {
    ...
    if (!senderRpc)
        missing.push('--sender-rpc');
    if (!senderChainId)
        missing.push('--sender-chain-id');
    if (!senderBlockchainId)
        missing.push('--sender-blockchain-id');
    if (!receiverRpc)
        missing.push('--receiver-rpc');
    if (!receiverChainId)
        missing.push('--receiver-chain-id');
    if (!receiverBlockchainId)
        missing.push('--receiver-blockchain-id');
    if (missing.length > 0) {
